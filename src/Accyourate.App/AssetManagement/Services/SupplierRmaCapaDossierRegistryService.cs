...
public sealed class SupplierRmaCapaDossierRegistryService
{
    ...
    private static SupplierRmaCapaDossierRegistryRecord ReadRecord(SqliteConnection connection,int id){using var command=connection.CreateCommand();command.CommandText="SELECT DocumentStatus,Custodian,Revision,RetentionUntil,ReviewOutcome,ReviewDueDate FROM SupplierRmaCapaDossierRegistry WHERE Id=$id;";command.Parameters.AddWithValue("$id",id);using var reader=command.ExecuteReader();if(!reader.Read())throw new InvalidOperationException("Registrazione non trovata.");return new(){DocumentStatus=S(reader,0),Custodian=S(reader,1),Revision=S(reader,2),RetentionUntil=S(reader,3),ReviewOutcome=S(reader,4),ReviewDueDate=S(reader,5)};}
