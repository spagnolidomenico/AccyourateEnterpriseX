...
public sealed class SupplierRmaPerformanceWindow : Window
{
    ...
    private static Control Header(){var g=GridRow();var names=new[]{"Fornitore","Pratiche","Aperte","Scadute","SLA","Tempo medio","Riparaz.","Sostituz.","Rimborsi","Costi","Affidabilità"};for(var i=0;i<names.Length;i++)Text(g,names[i],i,true);return new Border{Background=UiTokens.Brush(UiTokens.SurfaceAlt),Padding=new Thickness(8),Child=g};}
    private static Control Row(SupplierRmaPerformance x,int index){var g=GridRow();Text(g,x.SupplierName,0,true);Text(g,x.TotalCases.ToString(),1);Text(g,x.ActiveCases.ToString(),2);Text(g,x.OverdueCases.ToString(),3,x.OverdueCases>0);Text(g,x.EvaluatedSlaCases==0?"—":$"{x.SlaCompliancePercent:N0}%",4,true);Text(g,x.ClosedCases==0?"—":$"{x.AverageResolutionDays:N1} gg",5);Text(g,x.RepairCases.ToString(),6);Text(g,x.ReplacementCases.ToString(),7);Text(g,x.RefundCases.ToString(),8);Text(g,$"EUR {x.TotalCost:N2}",9);Add(g,Score(x.ReliabilityScore),10);return new Border{Background=UiTokens.Brush(index%2==0?UiTokens.Surface:UiTokens.SurfaceAlt),BorderBrush=UiTokens.Brush(UiTokens.Border),BorderThickness=new Thickness(0,0,0,1),Padding=new Thickness(8,7),Child=g};}
    ...
    private static Grid GridRow()=>new(){ColumnDefinitions=new ColumnDefinitions("210,75,70,75,80,105,80,85,75,115,110")};
