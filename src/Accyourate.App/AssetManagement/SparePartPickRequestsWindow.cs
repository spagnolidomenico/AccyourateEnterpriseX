...
public sealed class SparePartPickRequestsWindow : Window
{
    ...
    private async void Quarantine(){var owner=TopLevel.GetTopLevel(this) as Window;if(owner is null)return;await new SparePartQuarantineWindow().ShowDialog(owner);Load();}
...
public sealed class SparePartReturnDialog : Window
{
    ...
    public SparePartReturnDialog(SparePartPickRequest request,SparePartInventoryItem item,IReadOnlyList<SparePartWarehouseLocation> locations,decimal remaining)
    {
        ...
        Content=new ScrollViewer{Content=new StackPanel{Margin=new Thickness(24),Spacing=9,Children={new TextBlock{Text="Reso ricambio",FontSize=24,FontWeight=FontWeight.Bold},new TextBlock{Text=$"{request.RequestNumber} · {item.PartCode} · restituibile {remaining:N2}",Foreground=UiTokens.Brush(UiTokens.TextSecondary)},Field("Quantità",_quantity),Field("Condizione",_condition),Field("Ubicazione di rientro o quarantena",_location),Field("Motivo",_reason),Field("Note",_notes),_message,save}}};
    }
    private void Confirm(decimal remaining){if(!decimal.TryParse(_quantity.Text,out var quantity)||quantity<=0||quantity>remaining){Error($"Inserisci una quantità tra 0 e {remaining:N2}.");return;}var condition=_condition.SelectedItem?.ToString()??SparePartReturnCondition.Reusable;var locationId=_location.SelectedIndex>=0?_locations[_location.SelectedIndex].Id:0;if(locationId==0){Error(condition==SparePartReturnCondition.Reusable?"Seleziona l'ubicazione di rientro.":"Seleziona l'ubicazione di quarantena.");return;}Close(new SparePartReturn{Quantity=quantity,Condition=condition,LocationId=locationId,Reason=_reason.Text?.Trim()??"",Notes=_notes.Text?.Trim()??""});}
