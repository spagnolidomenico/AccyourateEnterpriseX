...
internal sealed class SupplierRmaCorrectiveActionAttachmentsWindow:Window
{
    ...
    private void ExportDossier(){try{var attachments=_service.GetAttachments(_action.Id);var path=new SupplierRmaCapaDossierService().Generate(_action,_service.GetHistory(_action.Id),attachments);new SupplierRmaCapaDossierRegistryService().RecordExport(_action,path,3+attachments.Count(x=>x.IsAvailable),Environment.UserName);Process.Start(new ProcessStartInfo{FileName=path,UseShellExecute=true});_message.Text=$"Fascicolo CAPA creato: {path}";_message.Foreground=UiTokens.Brush(UiTokens.Success);}catch(Exception ex){_message.Text=$"Fascicolo non creato: {ex.Message}";_message.Foreground=UiTokens.Brush(UiTokens.Danger);}}
    private async void VerifyDossier()
    {
        try
        {
            ...
            new SupplierRmaCapaDossierRegistryService().RecordVerification(_action,result,Environment.UserName);
