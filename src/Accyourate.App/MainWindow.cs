...
public sealed class MainWindow : Window
{
    ...
    private void SetContextArea(string area)
    {
        ...
        switch (area)
        {
            ...
            case "Asset":
                ...
                AddContextButton("📌 Inventario per ubicazione", OpenLocationStocktake);
    ...
    private void OpenSparePartsInventory()
    {
        if (_workspaceContent is null)
            return;
        const string key = "asset:spare-parts-inventory";
        if (string.Equals(_currentWorkspaceKey, key, StringComparison.Ordinal))
            return;
    ...
    private void OpenSparePartStocktake()
    {
        if (_workspaceContent is null)
            return;
        const string key = "asset:spare-parts-stocktake";
        if (string.Equals(_currentWorkspaceKey, key, StringComparison.Ordinal))
            return;
        _workspaceContent.Content = new SparePartStocktakeView();
        _currentWorkspaceKey = key;
        SetBreadcrumb("Workspace > Asset > Inventario fisico ricambi");
    }

    private void OpenSparePartLocations()
    {
        if (_workspaceContent is null)
            return;
    ...
    private void OpenLocationStocktake()
    {
        if (_workspaceContent is null)
            return;
        const string key = "asset:location-stocktake";
        if (string.Equals(_currentWorkspaceKey, key, StringComparison.Ordinal))
            return;
        _workspaceContent.Content = new LocationStocktakeView();
        _currentWorkspaceKey = key;
        SetBreadcrumb("Workspace > Asset > Inventario per ubicazione");
    }
